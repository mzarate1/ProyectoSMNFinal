...
public class ProvinciaController {
	...
	@Autowired
	private PronosticoServiceImpl servicioPronosticos;
	
	@Autowired
	private EventoServiceImpl servicioEventos;
...
List<Clima> listaClima = servicioClima.listarClimas();
		...
		{
			if(clima.getCiudad().getProvincia().getId_provincia() == id) 
			{
		...
		List<Pronostico> listaPronosticos = servicioPronosticos.listarPronosticos();
		
		for (Pronostico pronostico : listaPronosticos) 
		{
			if(pronostico.getId_ciudad().getProvincia().getId_provincia() == id) 
			{
				servicioPronosticos.eliminarPronostico(pronostico);
				
			}
		}
		
		List<Evento> listaEventos = servicioEventos.listarEvento();
		
		for (Evento evento : listaEventos) 
		{
			if(evento.getId_ciudad().getProvincia().getId_provincia() == id) 
			{
				servicioEventos.eliminarEvento(evento);
